var DZ = {
	...
	/**
	 * Invoked after loading the SDK on the app
	 *
	 * @param {Object} data - Options passed along when calling DZ.init on the app
	 */
...
DZ.deezer = {
	...
	follow: function(favorite_element) {
		...
		api.call('user.getData', {
			user_id: id,
			array_default: ['USER_ID', 'DISPLAY_NAME', 'USER_PICTURE']
		}, function(result, callback_parameters) {
			favorite.add({
		...
		callDeezer: function(json) {
			try {
				...
				if (json.method.substr(0,20) == 'DZ.player_controler.') {
					...
					Deezer.log('Method appel̩ ' + method);
